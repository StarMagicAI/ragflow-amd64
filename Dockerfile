...
RUN --mount=type=cache,id=ragflow_apt,target=/var/cache/apt,sharing=locked \
    ...
    if [ "$arch" = "arm64" ] || [ "$arch" = "aarch64" ]; then \
        ...
        ACCEPT_EULA=Y apt install -y msodbcsql18 || echo "Skipping msodbcsql18 install"; \
    else \
        # x86_64 or others
        ACCEPT_EULA=Y apt install -y msodbcsql17 || echo "Skipping msodbcsql17 install"; \
    fi



# Add dependencies of selenium - auto-detect architecture
RUN --mount=type=bind,from=infiniflow/ragflow_deps:latest,source=/,target=/deps \
    arch="$(uname -m)"; \
    if [ "$arch" = "aarch64" ] || [ "$arch" = "arm64" ]; then \
        # ARM64: use system chromium instead of pre-built Chrome (Chrome doesn't have ARM64 Linux build)
